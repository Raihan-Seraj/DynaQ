...
class Engine(object):
  ...
  * (Now is NOT the best time to read more about the `Plot`; for the time being,
  ...
  def its_showtime(self, ctr):
    ...
    return self.play(None, ctr)

  def play(self, actions, ctr):
    ...
    self._update_and_render(actions, ctr)
  ...
  def _update_and_render(self, actions, ctr):
    ...
    for update_group, entities in self._update_groups:
      ...
      for entity in entities:
        ...
        if hasattr(entity,'naaam'):
          entity.update(actions,
                self._board.board, self._board.layers,
                self._backdrop, self._sprites_and_drapes, self._the_plot, ctr)
        else:
